...
func TestPathIDsSplit(t *testing.T) {
	...
	for i := 1; i <= max; i++ {
		...
		h := c.config.AcquireHost(hostname)
		h.AddPath(b, "/")
		path := b.FindHostPath(hostname + "/")
		if i == 1 {
			// create the common config
			b.HSTS = append(b.HSTS, &hatypes.BackendConfigHSTS{
				Paths: hatypes.NewBackendPaths(b.FindHostPath(hostname + "/")),
				Config: hatypes.HSTS{
					Enabled: true,
				},
			})
...
func TestMatchRegexBackend(t *testing.T) {
	c := setup(t)
	defer c.teardown()
	...
	b = c.config.AcquireBackend("d1", "app", "8080")
	b.Endpoints = []*hatypes.Endpoint{endpointS1}
	h = c.config.AcquireHost("*.local")
	h.AddPath(b, "/api")
	h.AddPath(b, "/app")

	b.WhitelistHTTP = []*hatypes.BackendConfigWhitelist{
		{
			Paths:  hatypes.NewBackendPaths(b.FindHostPath("*.local/app")),
			Config: []string{"192.168.0.0/16"},
		},
		{
			Paths:  hatypes.NewBackendPaths(b.FindHostPath("*.local/api")),
			Config: []string{"10.0.0.0/8"},
		},
	}
...
backend d1_app_8080
    ...
    # path02 = *.local/app
    http-request set-var(txn.pathID) base,lower,map_beg(/etc/haproxy/maps/_back_d1_app_8080_idpath.map)
    http-request set-var(txn.pathID) base,lower,map_reg(/etc/haproxy/maps/_back_d1_app_8080_idpath_regex.map) if !{ var(txn.pathID) -m found }
    acl wlist_src0 src 192.168.0.0/16
    http-request deny if { var(txn.pathID) path02 } !wlist_src0
    acl wlist_src1 src 10.0.0.0/8
...
`)
	...
	c.checkMap("_back_d1_app_8080_idpath.map", ``)
	c.checkMap("_back_d1_app_8080_idpath_regex.map", `
^[^.]+\.local/app path02
^[^.]+\.local/api path01
`)
...
frontend _front002
    ...
    timeout client 2s
    http-request set-var(req.base) base,lower,regsub(:[0-9]+/,/)
    http-request set-var(req.hostbackend) var(req.base),map_beg(/etc/haproxy/maps/_front002_host.map)
    http-request set-var(req.host) hdr(host),lower,regsub(:[0-9]+$,)
    <<https-headers>>
    acl tls-has-crt ssl_c_used
    ...
    http-request set-var(req.path) path
    http-request set-var(req.snibase) ssl_fc_sni,concat(,req.path),lower
    http-request set-var(req.snibackend) var(req.snibase),map_beg(/etc/haproxy/maps/_front002_sni.map)
    http-request set-var(req.snibackend) var(req.base),map_beg(/etc/haproxy/maps/_front002_sni.map) if !{ var(req.snibackend) -m found } !tls-has-crt !tls-host-need-crt
...
func TestDNS(t *testing.T) {
	...
	b = c.config.AcquireBackend("d3", "app", "http")
	b.DNSPort = "named"
	b.Endpoints = []*hatypes.Endpoint{endpointS21, endpointS22}
	b.Resolver = "k8s"
	h = c.config.AcquireHost("d3.local")
...
frontend _front002
    ...
    http-request set-var(req.base) base,lower,regsub(:[0-9]+/,/)
    http-request set-var(req.hostbackend) var(req.base),map_beg(/etc/haproxy/maps/_front002_host.map)
    http-request set-var(req.hostbackend) var(req.base),map_reg(/etc/haproxy/maps/_front002_host_regex.map) if !{ var(req.hostbackend) -m found }
    http-request set-var(req.host) hdr(host),lower,regsub(:[0-9]+$,)
    http-request set-var(req.rootredir) var(req.host),map(/etc/haproxy/maps/_front002_root_redir.map)
    http-request set-var(req.rootredir) var(req.host),map_reg(/etc/haproxy/maps/_front002_root_redir_regex.map) if !{ var(req.rootredir) -m found }
    http-request redirect location %[var(req.rootredir)] if { path / } { var(req.rootredir) -m found }
    <<https-headers>>
    use_backend %[var(req.hostbackend)] if { var(req.hostbackend) -m found }
    default_backend _error404
