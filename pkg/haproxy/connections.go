...
func newConnections(masterSock, adminSock string, timeout time.Duration) *connections {
	return &connections{
		timeout:    timeout,
		mutex:      sync.Mutex{},
...
func (c *connections) TrackCurrentInstance(timeoutStopDur, closeSessDur time.Duration) error {
	...
	c.shrinkConns()
	sock := socket.NewSocketConcurrent(c.adminSock, c.timeout, true)
...
func (c *connections) Admin() socket.HAProxySocket {
	if c.admin == nil {
		c.admin = socket.NewSocket(c.adminSock, c.timeout, false)
	}
	return c.admin
}
...
func (c *connections) Master() socket.HAProxySocket {
	if c.master == nil {
		c.master = socket.NewSocket(c.masterSock, c.timeout, false)
	}
	return c.master
}
...
func (c *connections) DynUpdate() socket.HAProxySocket {
	if c.dynUpdate == nil {
		...
		c.dynUpdate = socket.NewSocket(c.adminSock, c.timeout, false)
	}
	return c.dynUpdate
}
...
func (c *connections) IdleChk() socket.HAProxySocket {
	if c.idleChk == nil {
		c.idleChk = socket.NewSocket(c.adminSock, c.timeout, false)
	}
	return c.idleChk
}
