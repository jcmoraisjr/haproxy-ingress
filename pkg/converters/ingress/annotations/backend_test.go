...
func TestRewriteURL(t *testing.T) {
	...
	for name, test := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			d.backend.AddBackendPath(hatypes.CreateHostPathLink("d1.local", test.ingPath, test.pathType))
			d.mapper.AddAnnotations(&source, hatypes.CreateHostPathLink("d1.local", test.ingPath, test.pathType), ann)
