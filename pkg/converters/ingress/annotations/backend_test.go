...
func TestRewriteURL(t *testing.T) {
	...
	for name, test := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			d.backend.AddBackendPath(hatypes.CreatePathLink("d1.local", test.ingPath, test.pathType))
			d.mapper.AddAnnotations(&source, hatypes.CreatePathLink("d1.local", test.ingPath, test.pathType), ann)
