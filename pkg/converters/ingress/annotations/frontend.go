...
func (c *frontData) get(key string) *ConfigValue {
	...
	if config.Source != nil && c.mapper.Get(ingtypes.FrontHTTPPortsLocal).Source == nil {
		// `config.Source != nil` -- annotation based config overriding global one; and
		// `mapper.Get("http-ports-local").Source == nil` -- local port config is missing.
		// malformed configuration, warn and use default/global instead
