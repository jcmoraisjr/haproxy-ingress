...
TLS configuration is also applied to the TCP service if configured, making HAProxy to ssl offload requests on that port. Up to `v0.14.7`, a single certificate can be configured for all incoming requests. Since `v0.14.8`, distinct TLS hosts sections can configure certificates per SNI, and the first declared secret act as the default certificate if an incoming SNI does not match any hosts entry. TLS related annotations are supported, and distinct configurations can be applied to distinct secrets by splitting the TCP service configuration into distinct ingress resources.
