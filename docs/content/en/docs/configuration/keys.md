...
`backend-server-rename`, when `true` and `backend-server-naming` is `ip` or `pod`, lets HAProxy Ingress rename a reused empty slot in place via the `set server <b>/<s> name` runtime command instead of triggering a reload, keeping the server name in sync with the workload now backing it. It defaults to `false` because the command requires HAProxy 3.5-dev2 or newer; on older versions the command is rejected and the update falls back to a reload.
